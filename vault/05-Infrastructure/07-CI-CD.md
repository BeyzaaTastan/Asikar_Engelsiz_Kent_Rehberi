...
- ✅ **Widget testleri** (cihazsız, `flutter test test/widget/`): `lib/screens/map/`'ten çıkarılan saf sunum widget'ları (monolit bölme — bkz. [[01-On-Yuz]]):
  ...
  - `SmartResultsOverlay` → `smart_results_overlay_test.dart` (5 senaryo): başlık (Son Aramalar↔Önerilen), boş geçmiş durumu, öğe listesi + `onItemTap`, yükleniyor spinner.
...
> Tek komutla çalışır: `flutter test` (90 senaryo — birim + widget; 2026-07-02: −3 `MapFilterChip` widget testi, +2 `accessibilityOverpassQuery` parking birim testi) ve `bash tool/run_rules_tests.sh` (10 senaryo — rules emulator) → sonradan CI'a sorunsuz bağlanır.
